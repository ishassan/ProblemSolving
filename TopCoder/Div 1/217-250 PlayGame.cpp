...
class PlayGame {
public:
    int get(int idx, int st){
        reps(i,st,sz(computer)){
            if(you[idx] > computer[i]) return i;
        }
        return -1;
    }

    int DP(int idx, int st, int rem){
        if(idx == sz(you)) return rem==0?0:-1<<27;
        int& m = memo[idx][st][rem];
        if(m != -1) return m;
        m = DP(idx+1,st,max(0,rem-1));
        int i = get(idx,st);
        if(i == -1) return m;
        m = max(m, DP(idx+1, i+1, rem+i-st)+you[idx]);
        return m;
    }
