...
public:
    int DP(int idx, int ques, int yes, int no){
        if(idx == sz(ans))
            return ques == 0;
        int& m = memo[idx][ques][yes][no];
        if(m != -1) return m;
        m = 0;
        if(ques)
            m += DP(idx+1, ques-1, yes+(ans[idx]=="Yes"),no + (ans[idx]=="No"))*ques;

        int mul = ans[idx]=="Yes"?yes:no;
        m += DP(idx+1, ques, yes, no)*mul;
        return m;
    }
