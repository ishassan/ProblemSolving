...
vector<vi> adj;
int subnodes(int idx, int prev, int n1, int n2, bool used){
    int res = used;
    for(int nxt : adj[idx]){
        if(nxt == prev)continue;
        bool u = (idx == n1 && nxt == n2) || (idx == n2 && nxt == n1);
        res += subnodes(nxt,idx,n1,n2,used||u);
    }
    return res;
}
...
public:
	double expectedLength(vector <int> A, vector <int> B, vector <int> f) {
        ...
        rep(i,sz(A)){
            double p=1.0;
            rep(j,sz(f)){
                double cnt = subnodes(f[j], -1, A[i], B[i], 0);
                p *= (cnt / sz(A));
            }
            res += p;
        }
...
bool KawigiEdit_RunTest(int testNum, vector <int> p0, vector <int> p1, vector <int> p2, bool hasAnswer, double p3) {
	...
	double answer;
	obj = new FoxAndTouristFamilies();
	clock_t startTime = clock();
	answer = obj->expectedLength(p0, p1, p2);
