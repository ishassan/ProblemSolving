...
public:
	...
	bool ls(vi A, vi B, vi want){
		rep(i,sz(want)){
			if(min(A[i], B[i]) != want[i])
				return false;
		}
		return true;
	}

	bool grt(vi A, vi B, vi want){
			rep(i,sz(want)){
				if(max(A[i], B[i]) != want[i])
					return false;
			}
			return true;
		}
	int makeProgram(vector <int> A, vector <int> B, vector <int> wanted) {
		...
		if(fA(A, wanted) || fB(B, wanted))
			return 1;

		if(ls(A,B,wanted) || grt(A,B,wanted))
			return 7;
		return -1;
	}
};
