...
public:
	int allocateTables(vector <int> tables, vector <int> groupSizes, vector <int> arrivals, vector <int> departures) {
        sort(all(tables));
        vector<pair<pii,int> > v(sz(groupSizes));
        rep(i,sz(groupSizes)){
            v.push_back(mp(mp(arrivals[i],departures[i]), groupSizes[i]));
        }
        pii vis[60];
        mem(vis,-1);
        int res=0;
        rep(i,sz(v)){
            int idx = lower_bound(all(tables),v[i].second) - tables.begin();
            if(idx == sz(tables)){
                res += v[i].second;
                continue;
            }
            while(idx < sz(tables)){
                if(vis[idx].first <= v[i].first.first && vis[idx].second > v[i].first.first){
                    ++idx;
                    continue;
                }
                vis[idx].first = v[i].first.first;
                vis[idx].second = v[i].first.second;
                goto end;
            }
            res += v[i].second;
            end:;
